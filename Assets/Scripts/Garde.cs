...
public class Garde : Character
{
    ...
    {
        base.Start();
        transform.position = _wayPoints[_startWaypointIndex].position;
    ...
    void Update()
    {
        Vector3 direction = _wayPoints[_currentIndexWaypoint].position - transform.position;
        
        transform.LookAt(new Vector3(_wayPoints[_currentIndexWaypoint].position.x,transform.position.y ,_wayPoints[_currentIndexWaypoint].position.z));
        
        _rb.velocity = direction.normalized * _speed;
        if (GetDistancePlane(transform.position,_wayPoints[_currentIndexWaypoint].position) < 0.5f)
        {
